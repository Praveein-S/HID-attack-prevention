...
fast_typing_count = 0  # Track consecutive fast typing
...
def on_press(key):
    ...
    if last_key_time:
        ...
        if time_diff < TYPING_THRESHOLD:
            fast_typing_count += 1  # Increment count of consecutive fast typing
            if fast_typing_count >= 2:  # Trigger action if 3 or more fast keystrokes
...
def open_notepad_and_focus():
    ...
    # Bring Notepad to the foreground using win32gui
    def bring_notepad_to_foreground():
        # Find the Notepad window
        hwnd = win32gui.FindWindow(None, "Untitled - Notepad")
        if hwnd != 0:
            # Set Notepad window to the foreground
